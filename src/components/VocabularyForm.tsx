...
interface VocabularyFormProps {
  onAdd: (vocab: Omit<Vocabulary, '_id'> & { _id?: string }) => Promise<void>;
...
export default function VocabularyForm({ onAdd, initialData, onCancelEdit }: VocabularyFormProps) {
  ...
  async function handleSubmit(e: React.SubmitEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await onAdd({
        ...
        engMeaning: engMeaning.trim(),
        burmeseMeaning: burmeseMeaning.trim(),
        cefr,
        wordForm,
      });
      ...
      console.error('Error submitting vocabulary form data:', error);
    } finally {
  ...
  return (
    <form
      ...
      className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 transition-all mb-8"
    >
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <div className="col-span-12 md:col-span-5 space-y-1.5">
          ...
          <input
            ...
            onChange={e => setBurmeseMeaning(e.target.value)}
        ...
        <div className="col-span-12 md:col-span-1 flex items-end pb-0.5">
          <button
            ...
            className="w-full h-9.5 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-bold text-xs uppercase tracking-widest rounded-md transition-all shadow-lg shadow-blue-100 disabled:shadow-none cursor-pointer disabled:cursor-not-allowed"
          >
            {isSubmitting ? (
